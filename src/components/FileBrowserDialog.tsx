...
export default function FileBrowserDialog({
  ...
  const showMobilePreview = isMobile && selectedPath;

  return (
    <Dialog open={open} onClose={onClose}>
      <div
        className={`flex w-full max-w-4xl flex-col rounded-xl bg-white dark:bg-zinc-800 shadow-xl mx-4 ${isMobile ? "h-[85dvh]" : ""}`}
        style={isMobile ? undefined : { height: "70vh", maxHeight: 600 }}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-700 px-4 md:px-5 py-3">
          <div className="flex items-center gap-2 min-w-0">
            {showMobilePreview ? (
              <button
                onClick={() => { setSelectedPath(null); setFileContent(null); setRawUrl(null); }}
          ...
          <button
            onClick={onClose}
            className="text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300 text-xl leading-none shrink-0"
          >
            &times;
          </button>
        </div>
